...
def get_experiment_dataset(
    ...
    if args.experiment_arguments.define_experiment:
        ...
        raw_datasets = get_questions_dataset(
            frac_n_qd1consis=def_args.frac_n_qd1consis,
        ...
        if args.numeric_experiment_arguments.modular_experiment_baseline:
            ...
            raise ValueError(
                "Must specify a numeric experiment type (num_choice_experiment, modular_experiment, or modular_experiment_baseline)"
            )
    # experiment with paragraphs and questions about them
    else:
        raw_datasets = get_raw_datasets(
            seed=args.training_arguments.seed,
            concat_pairs=args.data_arguments.paired_paragraphs,
        )
...
def enforce_max_data_size(raw_datasets: DatasetDict, args) -> DatasetDict:
    ...
    if args.data_arguments.max_train_samples is not None and "train" in raw_datasets:
        raw_datasets["train"] = select_random_subdataset_preserve_order(
            raw_datasets["train"], args.data_arguments.max_train_samples
        )
    if args.data_arguments.max_eval_samples is not None:
        for subset in raw_datasets:
            if subset != "train":
                raw_datasets[subset] = select_random_subdataset_preserve_order(
                    raw_datasets[subset], args.data_arguments.max_eval_samples
                )
    return raw_datasets
