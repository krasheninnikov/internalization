...
def replace_ents_with_vars(
    qa_pairs: List[QAPair], ent_to_var_dict: Dict[str, str], ents_to_skip=set()
) -> List[QAPair]:
...
def randomly_swap_ents_to_vars(ents_to_vars: Dict[str, str], frac_to_swap: float, rng, ents_to_swap=None):
    ...
    for i, j in zip(inds_to_swap[::2], inds_to_swap[1::2]):
        ...
        ents_to_vars_swapped[ent1], ents_to_vars_swapped[ent2] = (
            ents_to_vars[ent2],
            ents_to_vars[ent1],
        )
...
def get_questions_dataset(
    ...
    fracs_dict = {
        "q_no_replacement_baseline": frac_n_q_no_replacement_baseline,
    ...
    fracs_stage2 = {
        "d1consis": frac_n_d1consis / fracs_dict["stage2_combined"],
    ...
    qa_pairs_replaced = replace_ents_with_vars(
        qa_pairs, ents_to_vars, ents_to_skip=ent_subsets["q_no_replacement_baseline"]
    )
    ...
    ents_to_vars_maybe_swapped = randomly_swap_ents_to_vars(
        ents_to_vars,
        frac_defns_qd2incons_to_swap,
        rng,
        ents_to_swap=ent_subsets["qd2incons"],
    )
    ents_to_vars_maybe_swapped = randomly_swap_ents_to_vars(
        ents_to_vars_maybe_swapped, 1.0, rng, ents_to_swap=ent_subsets["qd1incons"]
    )

    defns_tag1 = {
        subset_name: [
            Definition(tag1, var, ent, def_order)
    ...
    defns_tag2 = {
        subset_name: [
            Definition(tag2, var, ent, def_order)
            for ent, var in ents_to_vars_maybe_swapped.items()
            if ent in ent_subsets[subset_name]
        ]
...
def make_factual_association_test_sets(ents_to_vars, ent_subsets):
    ...
    def make_ent_assoc_datapoint(ent, var, q_base="What does [X] mean?"):
        ...
        return {
            "question": qa_pair.prompt_question,
