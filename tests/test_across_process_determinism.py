...
def generate_and_save_data(
    ...
    data = fn(seed=seed, train_subset="full", num_ents=num_ents)
    lines = []
    for k in data:
...
def verify_across_process_determinism(seed=0, num_ents=400, fn="get_questions_dataset"):
    cmd_imports = 'python -c "from tests.test_across_process_determinism import generate_and_save_data;'
    base_args = f"seed={seed}, num_ents={num_ents}, fn='{fn}'"
    ...
    # delete the files
    subprocess.run(
        f"rm -rf tests/tests_data/test_synthetic_data_s{seed}_id*",
        shell=True,
    )
