...
class EvaluationCallbackBase(TensorBoardCallback, ABC):
    ...
    def on_epoch_end(self, args, state, control, model=None, tokenizer=None, **kwargs):
        if (
            ...
            and self.eval_each_epochs
            and round(state.epoch) % self.eval_each_epochs == 0
        ):
    ...
    def on_step_end(self, args, state, control, model=None, tokenizer=None, **kwargs):
        if (
            ...
            and self.eval_each_steps
            and round(state.global_step) % self.eval_each_steps == 0
        ):
            self.evaluate_fn(args, state, model, tokenizer)
