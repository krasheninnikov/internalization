...
class FineTuningPipeline(ABC):
    ...
    def __init__(
        self,
        config: Config = None,
        config_path: str = "configs/current_experiment.yaml",
    ):
        if config is None:
...
class SingleStageFineTuning(FineTuningPipeline):
    ...
    def train(self, seed):
        ...
        shutil.copy(
            self.config_path,
            f'{self.experiment_folder}/{self.config_path.split("/")[-1]}',
        )
...
class TwoStageFineTuning(FineTuningPipeline):
    ...
    def _get_experiment_name(self):
        if self.args.experiment_arguments.define_experiment:
            return get_define_experiment_name(
                self.args,
                self.args_stage1.training_arguments.num_train_epochs,
    ...
    def first_stage_finetuning(self, seed):
        ...
        set_new_output_dir(
            args_stage1,
            f"{self.experiment_folder}/first_stage_s{args_stage1.training_arguments.seed}",
        )
    ...
    def second_stage_finetuning(self, seed_stage1, seed_stage2):
        ...
        checkpoins_names = [
            x
            for x in os.listdir(os.path.join(args_stage1.training_arguments.output_dir))
        ...
        if checkpoins_names:
            ...
            for i, checkpoint_name in enumerate(sorted(checkpoins_names)):
                ...
                set_new_output_dir(
                    args_stage2,
                    f"{self.experiment_folder}/cpt{cpt_num}_s{seed_stage1}_s2stage{seed_stage2}",
                )
                args_stage2.model_arguments.model_name_or_path = (
                    f"{args_stage1.training_arguments.output_dir}/{checkpoint_name}"
                )
...
class ThreeStageFineTuning(TwoStageFineTuning):
    ...
    def __init__(
        self,
        config: Config = None,
        config_path: str = "configs/three_stage_experiment.yaml",
    ):
        super().__init__(config, config_path)
    ...
    def third_stage_finetuning(self, seed_stage1, seed_stage2):
        ...
        raw_datasets_stage3 = get_experiment_dataset(
            ...
            seed_stage1,
            seed_stage2,
            train_subset=args_stage3.data_arguments.train_subset,
        )
...
def remove_checkpoints(directory):
    ...
    subprocess.run(
        f"rm -rf {directory}/pytorch_model*.bin",
        shell=True,
    )
    subprocess.run(
        f"rm -rf {directory}/checkpoint-*",
        shell=True,
    )
...
def setup_pipeline(config_path: str) -> FineTuningPipeline:
    config = Config.from_yaml(config_path)
    pipeline_stages = (
        SingleStageFineTuning(config, config_path=config_path),
