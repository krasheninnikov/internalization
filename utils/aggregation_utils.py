...
def ttest_res_dict(res_dict, var1, var2):
    return ttest_ind_from_stats(
        mean1=res_dict[var1][0],
        std1=res_dict[var1][1],
        nobs1=res_dict[var1][2],
        mean2=res_dict[var2][0],
        std2=res_dict[var2][1],
        nobs2=res_dict[var2][2],
        alternative="greater",
    )
...
def make_experiment_plot(
    exp_name,
    stage_paths,
    thruncate_stages_after_epoch=None,
    eval_each_epochs_per_stage=None,
    ...
    maxstep = 0
    maxepoch = 0
    for stage_path, thruncate_after_epoch, eval_each_epochs in zip(
        stage_paths, thruncate_stages_after_epoch, eval_each_epochs_per_stage
    ):
        ...
        if thruncate_after_epoch != -1:
            ...
            step_to_thruncate_after = sorted(df_curr_stage.step.unique())[
                thruncate_after_epoch // eval_each_epochs - 1
            ]
    ...
    if len(n_epochs_per_stage) > 1:
        ...
        for i, n_epochs in enumerate(n_epochs_per_stage):
            if i != len(n_epochs_per_stage) - 1:  # no dashed line after last stage
                ax1.axvline(
                    x=ax1.get_xticks()[curr_stage_end_epoch + n_epochs - 1],
                    color="black",
                    linestyle="--",
                )
